...
class dictamen {
	...
	function cp_modelo() {
		...
		$opt = '';
		$ops = array('PRO', 'AME');
		for ($r = 0; $r < count($ops); $r++) { $opt .= '&' . $ops[$r] . ':' . msg('prj_type_' . $ops[$r]);
		}
	...
	function dictamen_recupera() {
		...
		$sql = "select * from cep_protocolos 
					inner join cep_parecer on cep_protocol = pr_protocol  
					where cep_protocol = '$caae' and pr_status <> 'C' ";
		$rlt = db_query($sql);
	...
	function dictamen_save($dd) {
		...
		$sql = "update cep_parecer set pr_status = 'C' where pr_protocol = '$caae' ";
		$rlt = db_query($sql);
